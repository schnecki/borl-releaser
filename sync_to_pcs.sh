...
for PC in $PCS; do
  ...
  rsync -tarz --del --force --exclude=.git --exclude=results --exclude=.stack-work --exclude=.stack-work.prof --exclude=state* --exclude=costs --exclude=plts --exclude=.state* --exclude=psiValues --exclude=.psiValues  --exclude=episodeLength --exclude=queueLength --exclude=reward $DIR $USER@$PC:$DIR
  if [ $? -ne 0 ]; then
